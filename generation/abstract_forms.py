...
simplex = normalize(
        noise.fractal_simplex_noise(
            noise="simplex", height=512, width=512, x_offset=0, y_offset=0, 
            scale=512, octaves=5, persistence=0.5, lacunarity=2.0
        )
)
eroded, sediment, gradient = erosion(simplex, iterations=100, verbose=True)
with open("generation/hold/sediment.pkl", "wb") as f:
    pickle.dump(sediment, f)
with open("generation/hold/gradient.pkl", "wb") as f:
    pickle.dump(gradient, f)

display = Display(height_array=eroded, height_scale=250, colormap="winter")
