...
def mask_splines(spline_points, strahler_numbers, min_x, min_y, max_x, max_y, river_width=1.5, scale_exponent=3, padding=50):
...
def carve_smooth_river_into_terrain(
    heightmap, 
    river_mask, 
    padding, original_width, original_height,
    water_threshold=0.2,
    river_depth_factor=0.7,    # Controls how deep the river is relative to water_threshold (smaller = deeper)
    influence_distance=40,     # How far the river's influence extends
    ...
    bank_sigma=5,              # Controls smoothness of the riverbanks
    bank_influence=0.8,        # How much the bank smoothing affects the final result
    depth_variation=0.3        # How much the depth varies between narrow and wide sections (0=uniform)
):
    """
    ...
    - bank_sigma: Controls smoothness of the riverbanks
    - bank_influence: How much the bank smoothing affects final result
    ...
    max_width = 32
    normalized_width = river_width / smooth_max(max_width, 1.0, 1)
    ...
    river_depth_map = min_river_depth - (normalized_width * (min_river_depth - max_river_depth))
    ...
    # Extract the original area from the padded mask
    new_heightmap = modified_heightmap[y_start:y_end, x_start:x_end]
...
heightmap00 = 0.8*(noise_generator.fractal_simplex_noise(noise="open", width=1024+2*padding, height=1024+2*padding,
                                                          scale=256, octaves=6, persistence=0.4, lacunarity=2.0, x_offset=0-padding, y_offset=0-padding)+1)/2
new_heightmap00 = carve_smooth_river_into_terrain(heightmap00, river_mask00, padding, original_width, original_height, influence_distance=20)

river_mask01, padding_info = mask_splines(splines, strahler_numbers, min_x=0, min_y=0+1024, max_x=1023, max_y=1023+1024)
padding, original_width, original_height = padding_info
heightmap01 = 0.8*(noise_generator.fractal_simplex_noise(noise="open", width=1024+2*padding, height=1024+2*padding,
                                                            scale=256, octaves=6, persistence=0.4, lacunarity=2.0, x_offset=0-padding, y_offset=1024-padding)+1)/2
new_heightmap01 = carve_smooth_river_into_terrain(heightmap01, river_mask01, padding, original_width, original_height, influence_distance=20)

river_mask10, padding_info = mask_splines(splines, strahler_numbers, min_x=0+1024, min_y=0, max_x=1023+1024, max_y=1023)
padding, original_width, original_height = padding_info
heightmap10 = 0.8*(noise_generator.fractal_simplex_noise(noise="open", width=1024+2*padding, height=1024+2*padding,
                                                            scale=256, octaves=6, persistence=0.4, lacunarity=2.0, x_offset=1024-padding, y_offset=0-padding)+1)/2
new_heightmap10 = carve_smooth_river_into_terrain(heightmap10, river_mask10, padding, original_width, original_height, influence_distance=20)
...
padding, original_width, original_height = padding_info
heightmap11 = 0.8*(noise_generator.fractal_simplex_noise(noise="open", width=1024+2*padding, height=1024+2*padding,
                                                            scale=256, octaves=6, persistence=0.4, lacunarity=2.0, x_offset=1024-padding, y_offset=1024-padding)+1)/2
new_heightmap11 = carve_smooth_river_into_terrain(heightmap11, river_mask11, padding, original_width, original_height, influence_distance=20)
...
# plt.imshow(full_heightmap, cmap='gray')
