...
def combine_heightmaps(old_heightmap, new_heightmap, old_sm, new_sm_blurred):
    ...
    # Only use prange on the outer loop for better parallelism
    for i in prange(height):
        # Use regular range for inner loop to avoid thread creation overhead
        for j in range(width):
            # Reduce memory access by caching values
            ...
            # Avoid branching with max by using a ternary-like approach
            ...
            # Single write operation to output
            blended_heightmap[i, j] = (old_heightmap[i, j] * norm_old + 
                                      new_heightmap[i, j] * norm_new)
            
            # Use simple comparison instead of min function
