...
def generate_dunes(frequency=10, noise_scale=5.0, noise_strength=200.0, rotation=-np.pi / 4, amplitude=1.5, gap=100):
    ...
    y = np.arange(height)
    X, Y = np.meshgrid(x, y)
    noise_values = noise.fractal_simplex_noise(
        noise="simplex", x_offset=0, y_offset=0, scale=width / noise_scale, octaves=1, persistence=0.5, lacunarity=2.0
    )
...
def volcanoes():
    ...
    simplex = normalize(
        noise.fractal_simplex_noise(
            noise="simplex", x_offset=0, y_offset=0, scale=256, octaves=10, persistence=0.5, lacunarity=2.0
        )
    )
...
def terraces():
    simplex = normalize(
        noise.fractal_simplex_noise(
            noise="simplex", x_offset=0, y_offset=0, scale=512, octaves=10, persistence=0.42, lacunarity=2.0
        )
    )
...
def phasor_dunes():
    dunes1 = normalize(
        generate_dunes(frequency=3, noise_scale=2.0, noise_strength=200.0, rotation=-np.pi / 4, amplitude=1, gap=3)
    )
    dunes2 = normalize(
        generate_dunes(frequency=6, noise_scale=2.0, noise_strength=500.0, rotation=-np.pi / 4, amplitude=1, gap=0.5)
    )
    dunes3 = normalize(
        generate_dunes(frequency=20, noise_scale=8.0, noise_strength=100.0, rotation=-np.pi / 4, amplitude=1, gap=1)
    )
    heightmap = normalize(1.3 * dunes1 + 0.2 * dunes2 + 0.05 * dunes3, 0, 0.5)

    phasor_noise = normalize(
        noise.phasor_noise(num_phasors=20, freq_range=(30, 40), amplitude=1.0, direction_bias=np.pi / 4, anisotropy=1)
    )
...
def pointy_mountains():
    ...
    simplex = normalize(
        noise.fractal_simplex_noise(
            noise="simplex", x_offset=0, y_offset=0, scale=512, octaves=10, persistence=0.5, lacunarity=2.0
        )
    )
    heightmap = normalize(blend(simplex, worley, alpha=0.5), a=0, b=1)

    warpx = noise.fractal_simplex_noise(
        noise="simplex", x_offset=0, y_offset=0, scale=128, octaves=2, persistence=0.5, lacunarity=2.0
    )
    warpy = noise.fractal_simplex_noise(
        noise="simplex", x_offset=0, y_offset=0, scale=128, octaves=2, persistence=0.5, lacunarity=2.0
    )
...
def terraced_plains():
    ...
    heightmap = normalize(0.5 * heightmap + 0.1 * add, 0.2, 0.7)
    return heightmap
...
def rocky_field():
    ...
    EDT Mask
    """
    base_noise = noise.fractal_simplex_noise(
        noise="simplex", x_offset=0, y_offset=0, scale=1024, octaves=2, persistence=0.5, lacunarity=2.0
    )
    base_noise = normalize(base_noise, 0, 1)

    rocky_noise = noise.fractal_simplex_noise(
        seed=100, noise="simplex", x_offset=0, y_offset=0, scale=512, octaves=10, persistence=0.5, lacunarity=2.2
    )
    rocky_noise = np.abs(normalize(rocky_noise, -1, 1))

    texture_noise = noise.fractal_simplex_noise(
        noise="simplex", x_offset=0, y_offset=0, scale=128, octaves=4, persistence=0.5, lacunarity=2.0
    )
...
def billowed_hills():
    base_noise = noise.fractal_simplex_noise(
        x_offset=0, y_offset=0, scale=1048, octaves=2, persistence=0.5, lacunarity=2.0
    )
    base_noise = normalize(base_noise, 0, 1)
    ...
    texture_noise = noise.fractal_simplex_noise(
        x_offset=0, y_offset=0, scale=128, octaves=4, persistence=0.5, lacunarity=2.0
    )
