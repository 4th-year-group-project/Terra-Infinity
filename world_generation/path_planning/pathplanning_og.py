...
def path_planning(lattice, Z, a, b, d, e, num_endpoints, n):
    ...
    dist_matrix, Z_predecessors = csgraph.shortest_path(
        lattice_matrix, indices=Z, directed=False, return_predecessors=True, method="D"
    )
    endpoint_times = 0
    rebuild_times = 0
    scipy_times = 0
    ...
    while d > e:
        ...
        if np.size(new_nodes) > 1:
            dist_matrix_new, predecessors_new = csgraph.shortest_path(
                lattice_matrix, indices=new_nodes, directed=False, return_predecessors=True, method="D"
            )
        ...
        if np.size(new_endpoints) > 0:
            dist_matrix_endpoints_new = csgraph.shortest_path(
                lattice_matrix, indices=new_endpoints, directed=False, return_predecessors=False, method="D"
            )
        ...
        t1 = time.time()
        source_and_endpoints, new_endpoints = find_endpoints(
            num_endpoints, possible_endpoints, dist_matrix, all_endpoints, dist_matrix_endpoints, d
        )
        t2 = time.time()
