...
def generate(n):
    ...
    image_path = f'heightmap.png'  # Replace with your image file path
    ...
    height_array = (height_array / 255.0) * height_scale  # Scale height between 0 and `height_scale`
    ...
    plotter.look_at()
    plotter.screenshot(f"./cdla_images/cdla{val}_birdseye.png")
    plotter.camera.Elevation(315) 
    plotter.camera.Zoom(1.2)
    plotter.screenshot(f"./cdla_images/cdla{val}_side.png")

    ##For Detail Maps
    # plotter.export(f"./heightmaps/heightmap{n}.x3d", binary=False)  # Export the 3D terrain to an X3D file
